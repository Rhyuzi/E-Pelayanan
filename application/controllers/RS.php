...
class RS extends CI_Controller {
	...
	public function index(){
		...
		$data['sub_judul'] = 'SKTM Rumah Sakit';
		...
		if (isset($_POST['next'])) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->load->view('sktm/syarat_rs', $data);
	...
	public function form(){
		...
		if (isset($_POST['print'])) {
			...
			$this->form_validation->set_rules('rs', 'Rumah Sakit', 'required');
		
			if ($this->form_validation->run() == FALSE) {
				$this->load->view('template/header', $data);
				$this->load->view('sktm/form_rs', $data);
				$this->load->view('template/footer');
			}else{
				$this->rs_model->tambah_data();
				redirect('rs/cetak');
			}
		}else{
			$this->load->view('template/header', $data);
	...
	public function cetak(){
		...
		//$data['hasil'] = 'Clear';
		$data['hasil'] = $this->rs_model->tampilkan();
		$this->load->view('sktm/cetak_rs', $data);
	}
	
	public function reprint($id){
		$data['judul'] = 'Tinjauan Ulang';
		//$data['hasil'] = 'Clear';
        $data['hasil'] = $this->db->get_where('rs', ['no_reg' => $id])->result_array();
		$this->load->view('sktm/cetak_rs', $data);
	}

	public function hapus($id){
		$this->db->where('no_reg', $id);
		$this->db->delete('rs', ['no_reg' => $id]);
	}

	public function edit($id){
		$data['judul'] = 'Buat Surat';
		$data['sub_judul'] = 'SKTM-RS';
		$data['sub_judul2'] = 'Pengubahan';
		//$data['data_lama'] = $this->rs_model->get_rs($id);
		$data['data_lama'] = $this->rs_model->get_rs_by_id($id);
		...
		if ($this->form_validation->run() == FALSE) {
			$this->load->view('template/header', $data);
			$this->load->view('sktm/edit_rs', $data);
			$this->load->view('template/footer');
		}else{
			$this->rs_model->ubah_data_rs($id);
			//pesan
